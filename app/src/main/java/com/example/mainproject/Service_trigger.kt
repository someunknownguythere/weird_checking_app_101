...
public class MyService : Service(){
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        var unlock: Boolean
        ShowLog(isAppRunning(applicationContext).toString())
            while(isAppRunning(applicationContext).toString() != "com.example.mainproject" && isAppRunning(applicationContext).toString() != null ) {
                unlock= sharedPreferences.getBoolean("unlock", false)
                var key : Boolean = sharedPreferences.getBoolean("key", false)
            var a: Long = totaltimevisible("com.facebook.katana")
            ShowLog(a.toString())
            ShowLog("ALO")
            ShowLog(isAppRunning(applicationContext).toString())
            ShowLog(totaltimevisible ("com.facebook.katana").toString())
            ShowLog(lay_gia_tri_thoi_gian("facebook").toString())
            Thread.sleep(1_000)
            ShowLog(check().toString() + "checkkkkkk")
                ShowLog(key.toString() + "keyyyyyyyy")
                ShowLog(unlock.toString()+ "unlockkkkk")
                if(check() && !key && !unlock) {
